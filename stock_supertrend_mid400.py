...
def get_topix_mid400():
    ...
    r = requests.get(url)
    with open(tse_xls, 'wb') as f:
        f.write(r.content)

    df = pd.read_excel(tse_xls)
    df = df.query('規模区分 == "TOPIX Mid400"')
    df = df[['コード', '銘柄名']].rename(columns = {'コード': 'symbol', '銘柄名': 'name'})
...
if __name__=='__main__':
    ...
    # ログの設定
    setup_logger()
    logger.info('start script {}'.format(__file__))
